...
cnt <- 1
a<-read_html(url) %>% html_nodes('h3.lister-item-header') #h3 노드를 따온 것. 이제 a로시작하는 nodeㄹ를 따야함.
tmp_list <- read_html(url) %>% html_nodes('h3.lister-item-header') %>% #node에서 .은 class를 의미
  html_nodes('a[href^="/title"]') %>% html_attr('href')#a로 시작하는데 '/title'을 포함하고있는 파트만 따와라?
...
for( i in 1:5){
  ...
  title_year <- tmp_content %>% html_nodes('div.title_wrapper > h1') %>% html_text %>% str_trim # >는 두 개를 한번에 처리. sub 관계를 의미/ title._wrapper의 하위노드 h1을 따와라.
  tmp_title <- substr(title_year, 1, nchar(title_year)-7)               #html_text는 뭐지     #str_trim은 공백 제거
  ...
  # Rating counts
  tmp_count <- tmp_content %>% html_nodes('span.small') %>% html_text #class가 small으로 되어있으므로 쉽게 가져올 수 있음?
  ...
  tmp_writer <- sub("Writers:\n", "", tmp_writer) #| 지우는 거 직접찾아서 쓰기 special character
